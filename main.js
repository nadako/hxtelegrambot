// Generated by Haxe 3.3.0 (git build development @ b638b82)
...
Main.__name__ = true;
Main.main = function() {
	var api = new telegram_Methods(new Connection(js_node_Fs.readFileSync("bottoken.txt",{ encoding : "utf8"})));
	var lastUpdate = -1;
	var update = null;
	update = function() {
		api.connection.execute("getUpdates",{ offset : lastUpdate + 1},function(result) {
			switch(result[1]) {
			case 0:
				console.log("OMG ERROR: " + Std.string(result[2]));
				break;
			case 1:
				var updates = result[2];
				var _g = 0;
				while(_g < updates.length) {
					var update1 = updates[_g];
					++_g;
					lastUpdate = update1.update_id;
					if(update1.message != null && update1.message.text != null) {
						api.connection.execute("sendMessage",{ text : "Я тут", reply_to_message_id : update1.message.message_id, chat_id : update1.message.chat.id},null);
					}
				}
				break;
			}
			haxe_Timer.delay(update,1000);
		});
	};
	update();
};
